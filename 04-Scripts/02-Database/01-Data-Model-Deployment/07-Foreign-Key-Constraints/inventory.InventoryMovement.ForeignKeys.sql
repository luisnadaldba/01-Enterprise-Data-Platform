    ...
    (
        ...
        INNER JOIN sys.columns AS referenced_column
            ON referenced_column.object_id = OBJECT_ID(N'catalog.ProductVariant')
        WHERE parent_column.object_id = OBJECT_ID(N'inventory.InventoryMovement')
        AND parent_column.name = N'INVMV_PRDVA_id'
        AND referenced_column.name = N'PRDVA_id'
        AND parent_column.system_type_id = referenced_column.system_type_id
        AND parent_column.max_length = referenced_column.max_length
        AND parent_column.precision = referenced_column.precision
        AND parent_column.scale = referenced_column.scale
        AND TYPE_NAME(parent_column.user_type_id) = N'int'
        AND TYPE_NAME(referenced_column.user_type_id) = N'int'
    )
    BEGIN
    ...
    SELECT
        @INVMV_PRDVA_FK_actual_name = fk.name,
        ...
        @INVMV_PRDVA_FK_actual_delete_action = fk.delete_referential_action_desc,
        @INVMV_PRDVA_FK_actual_update_action = fk.update_referential_action_desc,
        @INVMV_PRDVA_FK_actual_is_disabled = fk.is_disabled,
        @INVMV_PRDVA_FK_actual_is_not_trusted = fk.is_not_trusted,
    ...
    AND fk.name = @INVMV_PRDVA_FK_expected_name;


    IF @INVMV_PRDVA_FK_actual_name IS NOT NULL
    BEGIN
        IF @INVMV_PRDVA_FK_actual_parent_table = N'[inventory].[InventoryMovement]'
        AND @INVMV_PRDVA_FK_actual_parent_columns = N'INVMV_PRDVA_id'
        AND @INVMV_PRDVA_FK_actual_referenced_table = N'[catalog].[ProductVariant]'
        AND @INVMV_PRDVA_FK_actual_referenced_columns = N'PRDVA_id'
        AND @INVMV_PRDVA_FK_actual_delete_action = N'NO_ACTION'
        AND @INVMV_PRDVA_FK_actual_update_action = N'NO_ACTION'
        ...
        BEGIN
            ...
            PRINT N'            Actual Table                    : ' + COALESCE(@INVMV_PRDVA_FK_actual_parent_table, N'<NULL>');
            PRINT N'            Expected Column                 : INVMV_PRDVA_id';
            PRINT N'            Actual Column                   : '
                + COALESCE(REPLACE(@INVMV_PRDVA_FK_actual_parent_columns, N'|', N', '), N'<NULL>');
            PRINT N'            Expected Reference              : catalog.ProductVariant.PRDVA_id';
            PRINT N'            Actual Reference Table         : ' + COALESCE(@INVMV_PRDVA_FK_actual_referenced_table, N'<NULL>');
            PRINT N'            Actual Reference Column        : '
                + COALESCE(REPLACE(@INVMV_PRDVA_FK_actual_referenced_columns, N'|', N', '), N'<NULL>');
            PRINT N'            Expected ON DELETE              : NO ACTION';
            PRINT N'            Actual ON DELETE                : ' + COALESCE(@INVMV_PRDVA_FK_actual_delete_action, N'<NULL>');
            PRINT N'            Expected ON UPDATE              : NO ACTION';
            PRINT N'            Actual ON UPDATE                : ' + COALESCE(@INVMV_PRDVA_FK_actual_update_action, N'<NULL>');
            PRINT N'            Expected Disabled               : 0';
            PRINT N'            Actual Disabled                 : '
                + COALESCE(CONVERT(nvarchar(1), @INVMV_PRDVA_FK_actual_is_disabled), N'<NULL>');
            PRINT N'            Expected Not Trusted            : 0';
            PRINT N'            Actual Not Trusted              : '
                + COALESCE(CONVERT(nvarchar(1), @INVMV_PRDVA_FK_actual_is_not_trusted), N'<NULL>');
    ...
    BEGIN
        ...
        SELECT TOP (1)
            @INVMV_PRDVA_FK_equivalent_name = fk.name,
            @INVMV_PRDVA_FK_equivalent_delete_action = fk.delete_referential_action_desc,
            @INVMV_PRDVA_FK_equivalent_update_action = fk.update_referential_action_desc,
            @INVMV_PRDVA_FK_equivalent_is_disabled = fk.is_disabled,
            @INVMV_PRDVA_FK_equivalent_is_not_trusted = fk.is_not_trusted
        FROM sys.foreign_keys AS fk
        WHERE fk.parent_object_id = OBJECT_ID(N'inventory.InventoryMovement')
        ...
        BEGIN
            ...
            PRINT N'            Actual Name                    : ' + @INVMV_PRDVA_FK_equivalent_name;
            PRINT N'            Column                         : INVMV_PRDVA_id';
            PRINT N'            References                     : catalog.ProductVariant.PRDVA_id';
            PRINT N'            ON DELETE                      : '
                + COALESCE(@INVMV_PRDVA_FK_equivalent_delete_action, N'<NULL>');
            PRINT N'            ON UPDATE                      : '
                + COALESCE(@INVMV_PRDVA_FK_equivalent_update_action, N'<NULL>');
            PRINT N'            Disabled                       : '
                + COALESCE(CONVERT(nvarchar(1), @INVMV_PRDVA_FK_equivalent_is_disabled), N'<NULL>');
            PRINT N'            Not Trusted                    : '
                + COALESCE(CONVERT(nvarchar(1), @INVMV_PRDVA_FK_equivalent_is_not_trusted), N'<NULL>');
            PRINT N'            Existing constraint was preserved for review.';
        END
        ELSE
        BEGIN
            ...
            BEGIN
                SELECT
                    @INVMV_PRDVA_FK_conflict_parent =
                        ...
                        + QUOTENAME(OBJECT_NAME(fk.parent_object_id))
                FROM sys.foreign_keys AS fk
                WHERE fk.object_id = OBJECT_ID(N'inventory.FK_INVMV_PRDVA', N'F');

                PRINT N'        [!] Foreign key name conflict       : FK_INVMV_PRDVA';
                PRINT N'            Expected Table                  : inventory.InventoryMovement';
                PRINT N'            Existing Parent                 : '
                    + COALESCE(@INVMV_PRDVA_FK_conflict_parent, N'<UNKNOWN>');
    ...
    (
        ...
        FROM sys.columns AS parent_column
        INNER JOIN sys.columns AS referenced_column
            ON referenced_column.object_id = OBJECT_ID(N'inventory.InventoryMovementReason')
        WHERE parent_column.object_id = OBJECT_ID(N'inventory.InventoryMovement')
        AND parent_column.name = N'INVMV_INVMR_id'
        AND referenced_column.name = N'INVMR_id'
        AND parent_column.system_type_id = referenced_column.system_type_id
        AND parent_column.max_length = referenced_column.max_length
        AND parent_column.precision = referenced_column.precision
        AND parent_column.scale = referenced_column.scale
        AND TYPE_NAME(parent_column.user_type_id) = N'smallint'
        AND TYPE_NAME(referenced_column.user_type_id) = N'smallint'
    )
    BEGIN
    ...
    SELECT
        @INVMV_INVMR_FK_actual_name = fk.name,
        ...
        @INVMV_INVMR_FK_actual_referenced_table =
            QUOTENAME(OBJECT_SCHEMA_NAME(fk.referenced_object_id)) + N'.'
            + QUOTENAME(OBJECT_NAME(fk.referenced_object_id)),
        @INVMV_INVMR_FK_actual_delete_action = fk.delete_referential_action_desc,
        @INVMV_INVMR_FK_actual_update_action = fk.update_referential_action_desc,
        @INVMV_INVMR_FK_actual_is_disabled = fk.is_disabled,
        @INVMV_INVMR_FK_actual_is_not_trusted = fk.is_not_trusted,
    ...
    FROM sys.foreign_keys AS fk
    WHERE fk.parent_object_id = OBJECT_ID(N'inventory.InventoryMovement')
    AND fk.name = @INVMV_INVMR_FK_expected_name;


    IF @INVMV_INVMR_FK_actual_name IS NOT NULL
    BEGIN
        IF @INVMV_INVMR_FK_actual_parent_table = N'[inventory].[InventoryMovement]'
        AND @INVMV_INVMR_FK_actual_parent_columns = N'INVMV_INVMR_id'
        AND @INVMV_INVMR_FK_actual_referenced_table = N'[inventory].[InventoryMovementReason]'
        AND @INVMV_INVMR_FK_actual_referenced_columns = N'INVMR_id'
        AND @INVMV_INVMR_FK_actual_delete_action = N'NO_ACTION'
        AND @INVMV_INVMR_FK_actual_update_action = N'NO_ACTION'
        ...
        BEGIN
            ...
            PRINT N'            Expected Table                  : inventory.InventoryMovement';
            PRINT N'            Actual Table                    : ' + COALESCE(@INVMV_INVMR_FK_actual_parent_table, N'<NULL>');
            PRINT N'            Expected Column                 : INVMV_INVMR_id';
            PRINT N'            Actual Column                   : '
                + COALESCE(REPLACE(@INVMV_INVMR_FK_actual_parent_columns, N'|', N', '), N'<NULL>');
            PRINT N'            Expected Reference              : inventory.InventoryMovementReason.INVMR_id';
            PRINT N'            Actual Reference Table         : ' + COALESCE(@INVMV_INVMR_FK_actual_referenced_table, N'<NULL>');
            PRINT N'            Actual Reference Column        : '
                + COALESCE(REPLACE(@INVMV_INVMR_FK_actual_referenced_columns, N'|', N', '), N'<NULL>');
            PRINT N'            Expected ON DELETE              : NO ACTION';
            PRINT N'            Actual ON DELETE                : ' + COALESCE(@INVMV_INVMR_FK_actual_delete_action, N'<NULL>');
            PRINT N'            Expected ON UPDATE              : NO ACTION';
            PRINT N'            Actual ON UPDATE                : ' + COALESCE(@INVMV_INVMR_FK_actual_update_action, N'<NULL>');
            PRINT N'            Expected Disabled               : 0';
            PRINT N'            Actual Disabled                 : '
                + COALESCE(CONVERT(nvarchar(1), @INVMV_INVMR_FK_actual_is_disabled), N'<NULL>');
            PRINT N'            Expected Not Trusted            : 0';
            PRINT N'            Actual Not Trusted              : '
                + COALESCE(CONVERT(nvarchar(1), @INVMV_INVMR_FK_actual_is_not_trusted), N'<NULL>');
            PRINT N'            Existing constraint was preserved for review.';
        END;
    END
    ELSE
    BEGIN
        ...
        SELECT TOP (1)
            @INVMV_INVMR_FK_equivalent_name = fk.name,
            @INVMV_INVMR_FK_equivalent_delete_action = fk.delete_referential_action_desc,
            @INVMV_INVMR_FK_equivalent_update_action = fk.update_referential_action_desc,
            @INVMV_INVMR_FK_equivalent_is_disabled = fk.is_disabled,
            @INVMV_INVMR_FK_equivalent_is_not_trusted = fk.is_not_trusted
        FROM sys.foreign_keys AS fk
        WHERE fk.parent_object_id = OBJECT_ID(N'inventory.InventoryMovement')
        ...
        BEGIN
            ...
            PRINT N'            Actual Name                    : ' + @INVMV_INVMR_FK_equivalent_name;
            PRINT N'            Column                         : INVMV_INVMR_id';
            PRINT N'            References                     : inventory.InventoryMovementReason.INVMR_id';
            PRINT N'            ON DELETE                      : '
                + COALESCE(@INVMV_INVMR_FK_equivalent_delete_action, N'<NULL>');
            PRINT N'            ON UPDATE                      : '
                + COALESCE(@INVMV_INVMR_FK_equivalent_update_action, N'<NULL>');
            PRINT N'            Disabled                       : '
                + COALESCE(CONVERT(nvarchar(1), @INVMV_INVMR_FK_equivalent_is_disabled), N'<NULL>');
            PRINT N'            Not Trusted                    : '
                + COALESCE(CONVERT(nvarchar(1), @INVMV_INVMR_FK_equivalent_is_not_trusted), N'<NULL>');
            PRINT N'            Existing constraint was preserved for review.';
        END
        ELSE
        BEGIN
            ...
            BEGIN
                SELECT
                    @INVMV_INVMR_FK_conflict_parent =
                        ...
                        + QUOTENAME(OBJECT_NAME(fk.parent_object_id))
                FROM sys.foreign_keys AS fk
                WHERE fk.object_id = OBJECT_ID(N'inventory.FK_INVMV_INVMR', N'F');

                PRINT N'        [!] Foreign key name conflict       : FK_INVMV_INVMR';
                PRINT N'            Expected Table                  : inventory.InventoryMovement';
                PRINT N'            Existing Parent                 : '
                    + COALESCE(@INVMV_INVMR_FK_conflict_parent, N'<UNKNOWN>');
                PRINT N'            Constraint was not created. Manual review is required.';
            ...
            ALTER TABLE inventory.InventoryMovement
                ...
                FOREIGN KEY (INVMV_INVMR_id)
                REFERENCES inventory.InventoryMovementReason (INVMR_id);
    ...
    OR COL_LENGTH(N'inventory.InventoryMovement', N'INVMV_TRNIT_transaction_at') IS NULL
    BEGIN

        ;THROW 50952,
            N'Foreign key FK_INVMV_TRNIT cannot be deployed because the composite parent columns do not exist.',
            1;
    ...
    BEGIN
        ...
        ;THROW 50953,
            N'Foreign key FK_INVMV_TRNIT cannot be deployed because the composite referenced columns do not exist.',
            1;
    ...
    SELECT
        @INVMV_TRNIT_FK_actual_name = fk.name,
        @INVMV_TRNIT_FK_actual_delete_action = fk.delete_referential_action_desc,
        @INVMV_TRNIT_FK_actual_update_action = fk.update_referential_action_desc,
        @INVMV_TRNIT_FK_actual_is_disabled = fk.is_disabled,
        @INVMV_TRNIT_FK_actual_is_not_trusted = fk.is_not_trusted,
    ...
    IF @INVMV_TRNIT_FK_actual_name IS NULL
    BEGIN
        ALTER TABLE inventory.InventoryMovement
            WITH CHECK
        ...
        PRINT N'            Trusted                         : YES';
    END
    ELSE IF @INVMV_TRNIT_FK_actual_parent_columns = N'INVMV_TRNIT_id|INVMV_TRNIT_transaction_at'
        AND @INVMV_TRNIT_FK_actual_referenced_columns = N'TRNIT_id|TRNIT_transaction_at'
        AND @INVMV_TRNIT_FK_actual_delete_action = N'NO_ACTION'
        AND @INVMV_TRNIT_FK_actual_update_action = N'NO_ACTION'
    ...
    BEGIN
        ...
        PRINT N'            Actual Columns                  : '
            + COALESCE(REPLACE(@INVMV_TRNIT_FK_actual_parent_columns, N'|', N', '), N'<NULL>');
        PRINT N'            Expected Reference              : sales.TransactionItem(TRNIT_id, TRNIT_transaction_at)';
        PRINT N'            Actual Reference Columns        : '
            + COALESCE(REPLACE(@INVMV_TRNIT_FK_actual_referenced_columns, N'|', N', '), N'<NULL>');
        PRINT N'            Existing constraint was preserved for review.';
    END;
