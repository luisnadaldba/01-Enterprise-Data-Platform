    ...
    DECLARE @PAYRF_PAY_FK_expected_name sysname = N'FK_PAYRF_PAY';
    ...
    BEGIN
        ...
        ;THROW 50990, N'Foreign key FK_PAYRF_PAY cannot be deployed because payment.PaymentRefund does not exist.', 1;
    END;

    IF OBJECT_ID(N'payment.Payment', N'U') IS NULL
    BEGIN
        PRINT N'        [X] Foreign key dependency missing : payment.Payment';
        ;THROW 50991, N'Foreign key FK_PAYRF_PAY cannot be deployed because payment.Payment does not exist.', 1;
    END;

    IF COL_LENGTH(N'payment.PaymentRefund', N'PAYRF_PAY_id') IS NULL
    BEGIN
        PRINT N'        [X] Foreign key column missing     : PAYRF_PAY_id';
        ;THROW 50992, N'Foreign key FK_PAYRF_PAY cannot be deployed because PAYRF_PAY_id does not exist.', 1;
    END;

    IF COL_LENGTH(N'payment.Payment', N'PAY_id') IS NULL
    BEGIN
        PRINT N'        [X] Referenced column missing      : PAY_id';
        ;THROW 50993, N'Foreign key FK_PAYRF_PAY cannot be deployed because referenced column payment.Payment.PAY_id does not exist.', 1;
    END;
    ...
    (
        ...
        INNER JOIN sys.columns AS referenced_column
            ON referenced_column.object_id = OBJECT_ID(N'payment.Payment')
        WHERE parent_column.object_id = OBJECT_ID(N'payment.PaymentRefund')
        AND parent_column.name = N'PAYRF_PAY_id'
        AND referenced_column.name = N'PAY_id'
    ...
    BEGIN
        ...
        ;THROW 50994, N'Foreign key FK_PAYRF_PAY cannot be deployed because participating columns are incompatible.', 1;
    END;

    PRINT N'        [✓] Foreign key dependencies validated : FK_PAYRF_PAY';

    SELECT
        @PAYRF_PAY_FK_actual_name = fk.name,
        ...
        @PAYRF_PAY_FK_actual_delete_action = fk.delete_referential_action_desc,
        @PAYRF_PAY_FK_actual_update_action = fk.update_referential_action_desc,
        @PAYRF_PAY_FK_actual_is_disabled = fk.is_disabled,
        @PAYRF_PAY_FK_actual_is_not_trusted = fk.is_not_trusted,
    ...
    AND fk.name = @PAYRF_PAY_FK_expected_name;

    IF @PAYRF_PAY_FK_actual_name IS NOT NULL
    BEGIN
        IF @PAYRF_PAY_FK_actual_parent_table = N'[payment].[PaymentRefund]'
        AND @PAYRF_PAY_FK_actual_parent_columns = N'PAYRF_PAY_id'
        AND @PAYRF_PAY_FK_actual_referenced_table = N'[payment].[Payment]'
        AND @PAYRF_PAY_FK_actual_referenced_columns = N'PAY_id'
        AND @PAYRF_PAY_FK_actual_delete_action = N'NO_ACTION'
        AND @PAYRF_PAY_FK_actual_update_action = N'NO_ACTION'
        ...
        BEGIN
            ...
            PRINT N'            Actual Table                    : ' + COALESCE(@PAYRF_PAY_FK_actual_parent_table, N'<NULL>');
            PRINT N'            Expected Column                 : PAYRF_PAY_id';
            PRINT N'            Actual Column                   : ' + COALESCE(REPLACE(@PAYRF_PAY_FK_actual_parent_columns, N'|', N', '), N'<NULL>');
            PRINT N'            Expected Reference              : payment.Payment.PAY_id';
            PRINT N'            Actual Reference Table         : ' + COALESCE(@PAYRF_PAY_FK_actual_referenced_table, N'<NULL>');
            PRINT N'            Actual Reference Column        : ' + COALESCE(REPLACE(@PAYRF_PAY_FK_actual_referenced_columns, N'|', N', '), N'<NULL>');
    ...
    BEGIN
        SELECT TOP (1)
            @PAYRF_PAY_FK_equivalent_name = fk.name
        FROM sys.foreign_keys AS fk
        WHERE fk.parent_object_id = OBJECT_ID(N'payment.PaymentRefund')
        ...
        AND (SELECT COUNT(*) FROM sys.foreign_key_columns AS fkc WHERE fkc.constraint_object_id = fk.object_id) = 1
        AND EXISTS
        (
            SELECT 1
            FROM sys.foreign_key_columns AS fkc
        ...
        BEGIN
            ...
            PRINT N'            Actual Name                    : ' + @PAYRF_PAY_FK_equivalent_name;
            PRINT N'            Column                         : PAYRF_PAY_id';
            PRINT N'            References                     : payment.Payment.PAY_id';
            PRINT N'            Existing constraint was preserved for review.';
        END
        ...
        BEGIN
            ...
            BEGIN
                SELECT @PAYRF_PAY_FK_conflict_parent =
                    QUOTENAME(OBJECT_SCHEMA_NAME(fk.parent_object_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id))
                FROM sys.foreign_keys AS fk
                WHERE fk.object_id = OBJECT_ID(N'payment.FK_PAYRF_PAY', N'F');

                PRINT N'        [!] Foreign key name conflict       : FK_PAYRF_PAY';
                PRINT N'            Expected Table                  : payment.PaymentRefund';
                PRINT N'            Existing Parent                 : ' + COALESCE(@PAYRF_PAY_FK_conflict_parent, N'<UNKNOWN>');
                PRINT N'            Constraint was not created. Manual review is required.';
                ;THROW 50995, N'Foreign key name conflict prevents safe deployment.', 1;
            END;

            ALTER TABLE payment.PaymentRefund
                WITH CHECK
                ADD CONSTRAINT FK_PAYRF_PAY
                FOREIGN KEY (PAYRF_PAY_id)
                REFERENCES payment.Payment (PAY_id);
    ...
    DECLARE @PAYRF_PAYRR_FK_expected_name sysname = N'FK_PAYRF_PAYRR';
    ...
    BEGIN
        PRINT N'        [X] Foreign key dependency missing : payment.PaymentRefund';
        ;THROW 51000, N'Foreign key FK_PAYRF_PAYRR cannot be deployed because payment.PaymentRefund does not exist.', 1;
    END;
    ...
    BEGIN
        ...
        ;THROW 51001, N'Foreign key FK_PAYRF_PAYRR cannot be deployed because payment.PaymentRefundReason does not exist.', 1;
    END;

    IF COL_LENGTH(N'payment.PaymentRefund', N'PAYRF_PAYRR_id') IS NULL
    BEGIN
        PRINT N'        [X] Foreign key column missing     : PAYRF_PAYRR_id';
        ;THROW 51002, N'Foreign key FK_PAYRF_PAYRR cannot be deployed because PAYRF_PAYRR_id does not exist.', 1;
    END;

    IF COL_LENGTH(N'payment.PaymentRefundReason', N'PAYRR_id') IS NULL
    BEGIN
        PRINT N'        [X] Referenced column missing      : PAYRR_id';
        ;THROW 51003, N'Foreign key FK_PAYRF_PAYRR cannot be deployed because referenced column payment.PaymentRefundReason.PAYRR_id does not exist.', 1;
    END;
    ...
    (
        ...
        FROM sys.columns AS parent_column
        INNER JOIN sys.columns AS referenced_column
            ON referenced_column.object_id = OBJECT_ID(N'payment.PaymentRefundReason')
        WHERE parent_column.object_id = OBJECT_ID(N'payment.PaymentRefund')
        AND parent_column.name = N'PAYRF_PAYRR_id'
        AND referenced_column.name = N'PAYRR_id'
        AND parent_column.system_type_id = referenced_column.system_type_id
        AND parent_column.max_length = referenced_column.max_length
        AND parent_column.precision = referenced_column.precision
        AND parent_column.scale = referenced_column.scale
        AND TYPE_NAME(parent_column.user_type_id) = N'tinyint'
        AND TYPE_NAME(referenced_column.user_type_id) = N'tinyint'
    )
    BEGIN
        PRINT N'        [X] Foreign key column mismatch    : PAYRF_PAYRR_id -> PAYRR_id';
        ;THROW 51004, N'Foreign key FK_PAYRF_PAYRR cannot be deployed because participating columns are incompatible.', 1;
    END;

    PRINT N'        [✓] Foreign key dependencies validated : FK_PAYRF_PAYRR';

    SELECT
        @PAYRF_PAYRR_FK_actual_name = fk.name,
        @PAYRF_PAYRR_FK_actual_parent_table =
            QUOTENAME(OBJECT_SCHEMA_NAME(fk.parent_object_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)),
        @PAYRF_PAYRR_FK_actual_referenced_table =
            QUOTENAME(OBJECT_SCHEMA_NAME(fk.referenced_object_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.referenced_object_id)),
        @PAYRF_PAYRR_FK_actual_delete_action = fk.delete_referential_action_desc,
        @PAYRF_PAYRR_FK_actual_update_action = fk.update_referential_action_desc,
        @PAYRF_PAYRR_FK_actual_is_disabled = fk.is_disabled,
        @PAYRF_PAYRR_FK_actual_is_not_trusted = fk.is_not_trusted,
    ...
    FROM sys.foreign_keys AS fk
    WHERE fk.parent_object_id = OBJECT_ID(N'payment.PaymentRefund')
    AND fk.name = @PAYRF_PAYRR_FK_expected_name;

    IF @PAYRF_PAYRR_FK_actual_name IS NOT NULL
    BEGIN
        IF @PAYRF_PAYRR_FK_actual_parent_table = N'[payment].[PaymentRefund]'
        AND @PAYRF_PAYRR_FK_actual_parent_columns = N'PAYRF_PAYRR_id'
        AND @PAYRF_PAYRR_FK_actual_referenced_table = N'[payment].[PaymentRefundReason]'
        AND @PAYRF_PAYRR_FK_actual_referenced_columns = N'PAYRR_id'
        AND @PAYRF_PAYRR_FK_actual_delete_action = N'NO_ACTION'
        AND @PAYRF_PAYRR_FK_actual_update_action = N'NO_ACTION'
        ...
        BEGIN
            ...
            PRINT N'            Expected Table                  : payment.PaymentRefund';
            PRINT N'            Actual Table                    : ' + COALESCE(@PAYRF_PAYRR_FK_actual_parent_table, N'<NULL>');
            PRINT N'            Expected Column                 : PAYRF_PAYRR_id';
            PRINT N'            Actual Column                   : ' + COALESCE(REPLACE(@PAYRF_PAYRR_FK_actual_parent_columns, N'|', N', '), N'<NULL>');
            PRINT N'            Expected Reference              : payment.PaymentRefundReason.PAYRR_id';
            PRINT N'            Actual Reference Table         : ' + COALESCE(@PAYRF_PAYRR_FK_actual_referenced_table, N'<NULL>');
            PRINT N'            Actual Reference Column        : ' + COALESCE(REPLACE(@PAYRF_PAYRR_FK_actual_referenced_columns, N'|', N', '), N'<NULL>');
            PRINT N'            Existing constraint was preserved for review.';
        END;
    END
    ELSE
    BEGIN
        SELECT TOP (1)
            @PAYRF_PAYRR_FK_equivalent_name = fk.name
        FROM sys.foreign_keys AS fk
        WHERE fk.parent_object_id = OBJECT_ID(N'payment.PaymentRefund')
        ...
        BEGIN
            ...
            PRINT N'            Actual Name                    : ' + @PAYRF_PAYRR_FK_equivalent_name;
            PRINT N'            Column                         : PAYRF_PAYRR_id';
            PRINT N'            References                     : payment.PaymentRefundReason.PAYRR_id';
            PRINT N'            Existing constraint was preserved for review.';
        END
        ...
        BEGIN
            ...
            BEGIN
                SELECT @PAYRF_PAYRR_FK_conflict_parent =
                    QUOTENAME(OBJECT_SCHEMA_NAME(fk.parent_object_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id))
                FROM sys.foreign_keys AS fk
                WHERE fk.object_id = OBJECT_ID(N'payment.FK_PAYRF_PAYRR', N'F');

                PRINT N'        [!] Foreign key name conflict       : FK_PAYRF_PAYRR';
                PRINT N'            Expected Table                  : payment.PaymentRefund';
                PRINT N'            Existing Parent                 : ' + COALESCE(@PAYRF_PAYRR_FK_conflict_parent, N'<UNKNOWN>');
                PRINT N'            Constraint was not created. Manual review is required.';
                ;THROW 51005, N'Foreign key name conflict prevents safe deployment.', 1;
            END;

            ALTER TABLE payment.PaymentRefund
                WITH CHECK
                ADD CONSTRAINT FK_PAYRF_PAYRR
                FOREIGN KEY (PAYRF_PAYRR_id)
                REFERENCES payment.PaymentRefundReason (PAYRR_id);
