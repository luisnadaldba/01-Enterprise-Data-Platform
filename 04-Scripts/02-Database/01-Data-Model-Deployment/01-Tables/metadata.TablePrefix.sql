    ...
    BEGIN
        ...
        PRINT N'            [•] Primary key              : PK_PFX';
        PRINT N'            [•] Column                   : PFX_id';
        PRINT N'            [•] Column                   : PFX_schema_name';
        PRINT N'            [•] Column                   : PFX_table_name';
        PRINT N'            [•] Column                   : PFX_prefix';
        PRINT N'            [•] Column                   : PFX_is_active';
        PRINT N'            [•] Column                   : PFX_created_at';
        PRINT N'            [•] Column                   : PFX_updated_at';
    END
    ELSE
    BEGIN
        ...
        (
            ...
            WHERE kc.parent_object_id = OBJECT_ID(N'metadata.TablePrefix')
              AND kc.type = N'PK'
              AND kc.name = N'PK_PFX'
              AND c.name = N'PFX_id'
        ...
        BEGIN
            PRINT N'            [X] Primary key mismatch        : PK_PFX';

            THROW 50003,
                  N'Primary key PK_PFX does not match the expected definition.',
                  1;
        END;
        ...
        IF COL_LENGTH(N'metadata.TablePrefix', N'PFX_schema_name') IS NULL
        BEGIN
            ALTER TABLE metadata.TablePrefix
                ADD PFX_schema_name sysname NULL;

            PRINT N'            [+] Column added               : PFX_schema_name';
            PRINT N'                [!] Nullability           : NULL (expected final state: NOT NULL)';
        END
        ELSE IF NOT EXISTS
        (
            SELECT 1
        ...
        IF COL_LENGTH(N'metadata.TablePrefix', N'PFX_table_name') IS NULL
        BEGIN
            ALTER TABLE metadata.TablePrefix
        ...
        IF COL_LENGTH(N'metadata.TablePrefix', N'PFX_prefix') IS NULL
        BEGIN
            ALTER TABLE metadata.TablePrefix
        ...
        IF COL_LENGTH(N'metadata.TablePrefix', N'PFX_is_active') IS NULL
        BEGIN
            ALTER TABLE metadata.TablePrefix
        ...
        IF COL_LENGTH(N'metadata.TablePrefix', N'PFX_created_at') IS NULL
        BEGIN
            ALTER TABLE metadata.TablePrefix
        ...
        IF COL_LENGTH(N'metadata.TablePrefix', N'PFX_updated_at') IS NULL
        BEGIN
            ALTER TABLE metadata.TablePrefix
