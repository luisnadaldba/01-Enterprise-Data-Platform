    ...
    BEGIN
        ...
        SELECT
            @PAYRF_ActualPrimaryKeyName = kc.name
        FROM sys.key_constraints AS kc
        INNER JOIN sys.indexes AS i
            ON i.object_id = kc.parent_object_id
            AND i.index_id = kc.unique_index_id
        WHERE kc.parent_object_id = OBJECT_ID(N'payment.PaymentRefund')
        AND kc.type = N'PK';
        ...
        IF @PAYRF_ActualPrimaryKeyName <> N'PK_PAYRF'
        BEGIN
            PRINT N'            [!] Primary key naming divergence :';
            PRINT N'                Expected                     : PK_PAYRF';
            PRINT N'                Actual                       : ' + @PAYRF_ActualPrimaryKeyName;
        ...
        IF COL_LENGTH(N'payment.PaymentRefund', N'PAYRF_amount') IS NULL
        BEGIN
            ALTER TABLE payment.PaymentRefund
        ...
        IF COL_LENGTH(N'payment.PaymentRefund', N'PAYRF_refunded_at') IS NULL
        BEGIN
            ALTER TABLE payment.PaymentRefund
        ...
        IF COL_LENGTH(N'payment.PaymentRefund', N'PAYRF_created_at') IS NULL
        BEGIN
            ALTER TABLE payment.PaymentRefund
        ...
        IF COL_LENGTH(N'payment.PaymentRefund', N'PAYRF_updated_at') IS NULL
        BEGIN
            ALTER TABLE payment.PaymentRefund
