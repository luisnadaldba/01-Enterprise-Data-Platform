...
/*==============================================================================
    DEFAULT CONSTRAINT: DF_TRN_shipping_amount
==============================================================================*/

SET @TRN_default_expected_name = N'DF_TRN_shipping_amount';
...
/*----------------------------------------------------------------------
    VALIDATE DEFAULT CURRENTLY ASSOCIATED WITH TRN_shipping_amount
----------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------
    NO DEFAULT CURRENTLY EXISTS ON TRN_shipping_amount
----------------------------------------------------------------------*/

IF @TRN_default_actual_name IS NULL
BEGIN

    IF OBJECT_ID(N'sales.DF_TRN_shipping_amount', N'D') IS NOT NULL
    BEGIN
        SELECT
            @TRN_default_parent_object =
                QUOTENAME(OBJECT_SCHEMA_NAME(dc.parent_object_id))
        ...
        FROM sys.default_constraints AS dc
        WHERE dc.object_id =
            OBJECT_ID(N'sales.DF_TRN_shipping_amount', N'D');

        PRINT N'        [!] Default constraint name conflict : DF_TRN_shipping_amount';
        PRINT N'            Expected Table                  : sales.Transaction';
        ...
        ;THROW 50047,
            N'Default constraint DF_TRN_shipping_amount already exists on another object.',
            1;
    END;

    ALTER TABLE sales.[Transaction]
        ADD CONSTRAINT DF_TRN_shipping_amount
        DEFAULT (0.00) FOR TRN_shipping_amount;

    PRINT N'        [+] Default constraint added        : DF_TRN_shipping_amount';
...
BEGIN
    ...
    IF @TRN_default_actual_name = @TRN_default_expected_name
       AND TRY_CONVERT(decimal(19,2), @TRN_default_normalized_definition) = 0.00
    BEGIN
        PRINT N'        [•] Default constraint validated    : DF_TRN_shipping_amount';
        PRINT N'            Column                          : TRN_shipping_amount';
        PRINT N'            Definition                      : DEFAULT (0.00)';
    END
    ...
    ELSE
    BEGIN
        PRINT N'        [!] Default constraint mismatch     : TRN_shipping_amount';
        PRINT N'            Expected Name                   : DF_TRN_shipping_amount';
        PRINT N'            Actual Name                     : '
            + COALESCE(@TRN_default_actual_name, N'<NULL>');
        PRINT N'            Expected Definition             : DEFAULT (0.00)';
        PRINT N'            Actual Definition               : '
