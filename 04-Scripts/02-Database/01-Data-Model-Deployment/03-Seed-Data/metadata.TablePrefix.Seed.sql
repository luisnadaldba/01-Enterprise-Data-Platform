    ...
    /*----------------------------------------------------------------------
        metadata.TablePrefix -> metadata.TablePrefix / PFX
    ----------------------------------------------------------------------*/
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT
                @PFX_actual_prefix    = PFX_prefix,
                @PFX_actual_is_active = PFX_is_active
            FROM metadata.TablePrefix
            WHERE PFX_schema_name = N'metadata'
              AND PFX_table_name = N'TablePrefix';
            ...
            PRINT N'            Actual Active                : '
                + COALESCE(CONVERT(nvarchar(1), @PFX_actual_is_active), N'<NULL>');
