    ...
    /*==============================================================================
        CHECK CONSTRAINT: CK_TRN_shipping_amount
    ==============================================================================*/

    SET @TRN_check_expected_name = N'CK_TRN_shipping_amount';
    ...
    FROM sys.check_constraints AS cc
    WHERE cc.parent_object_id = OBJECT_ID(N'sales.[Transaction]')
    AND cc.definition LIKE N'%TRN_shipping_amount%';


    IF @TRN_check_actual_name IS NULL
    BEGIN

        IF OBJECT_ID(N'sales.CK_TRN_shipping_amount', N'C') IS NOT NULL
        BEGIN
            SELECT
                @TRN_check_parent_object =
                    QUOTENAME(OBJECT_SCHEMA_NAME(cc.parent_object_id))
            ...
            FROM sys.check_constraints AS cc
            WHERE cc.object_id =
                OBJECT_ID(N'sales.CK_TRN_shipping_amount', N'C');

            PRINT N'        [!] Check constraint name conflict : CK_TRN_shipping_amount';
            PRINT N'            Expected Table                : sales.Transaction';
            PRINT N'            Expected Column               : TRN_shipping_amount';
            PRINT N'            Existing Parent               : '
                + COALESCE(@TRN_check_parent_object, N'<UNKNOWN>');
            PRINT N'            Constraint was not created. Manual review is required.';

            ;THROW 50052,
                N'Check constraint CK_TRN_shipping_amount already exists on another object.',
                1;
        END;

        ALTER TABLE sales.[Transaction] WITH CHECK
            ADD CONSTRAINT CK_TRN_shipping_amount
            CHECK (TRN_shipping_amount >= 0.00);

        PRINT N'        [+] Check constraint added         : CK_TRN_shipping_amount';
        PRINT N'            Column                         : TRN_shipping_amount';
        PRINT N'            Definition                     : CHECK (TRN_shipping_amount >= 0.00)';
    END
    ...
    BEGIN
        ...
        IF @TRN_check_actual_name = @TRN_check_expected_name
        AND @TRN_check_normalized_definition LIKE N'%trn_shipping_amount>=(0.00)%'
        AND @TRN_check_is_disabled = 0
        AND @TRN_check_is_not_trusted = 0
        BEGIN
            PRINT N'        [•] Check constraint validated     : CK_TRN_shipping_amount';
            PRINT N'            Column                         : TRN_shipping_amount';
            PRINT N'            Definition                     : CHECK (TRN_shipping_amount >= 0.00)';
        END
        ELSE
        BEGIN
            PRINT N'        [!] Check constraint mismatch      : TRN_shipping_amount';
            PRINT N'            Expected Name                  : CK_TRN_shipping_amount';
            PRINT N'            Actual Name                    : '
                + COALESCE(@TRN_check_actual_name, N'<NULL>');
            PRINT N'            Expected Definition            : CHECK (TRN_shipping_amount >= 0.00)';
            PRINT N'            Actual Definition              : '
                + COALESCE(@TRN_check_actual_definition, N'<NULL>');
            PRINT N'            Is Disabled                    : '
                + COALESCE(CONVERT(nvarchar(1), @TRN_check_is_disabled), N'<NULL>');
