    ...
    BEGIN
        ...
        SELECT
            @TRNIT_existing_description = CONVERT(nvarchar(4000), ep.value)
        FROM sys.extended_properties AS ep
        WHERE ep.class = 1
        AND ep.major_id = OBJECT_ID(N'sales.TransactionItem')
        AND ep.minor_id = 0
        ...
        IF @TRNIT_existing_description = @TRNIT_expected_description
        BEGIN
        ...
        BEGIN
            ...
            PRINT N'            Expected                     : ' + @TRNIT_expected_description;
            PRINT N'            Actual                       : '
                + CASE
                    WHEN @TRNIT_existing_description IS NULL THEN N'<NULL>'
                    WHEN LEN(@TRNIT_existing_description) = 0 THEN N'<EMPTY>'
