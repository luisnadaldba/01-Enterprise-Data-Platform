...
It documents the implemented database objects directly from validated SQL Server metadata and provides a structured reference for schemas, tables, columns, data types, nullability, identity properties, default constraints, controlled seed data, keys, constraints, foreign key relationships, indexes, partitioning, triggers, object descriptions, and technical governance metadata.
...
Metadata is extracted directly from SQL Server system catalogs and extended properties and is validated incrementally before being incorporated into this document.
...
- Change Tracking;
- transactional, snapshot, or merge replication;
...
These items are intentionally absent from the current AtlasCommerce transactional database implementation unless introduced by a future architectural requirement.
...
The data dictionary documents the following metadata categories:

- table description;
- columns in physical ordinal order;
- data type;
- nullability;
- identity property;
- default constraint;
...
The values documented in this section represent the currently implemented seed data extracted directly from the AtlasCommerce database.

Identifiers and controlled values are documented exactly as stored in the database.
...
The registry establishes the authoritative relationship between each table and its assigned column prefix.

At the time of this documentation:

- 41 user tables are implemented;
- 41 table-prefix assignments are registered;
- every implemented user table has a corresponding prefix assignment;
- every registered prefix assignment is active.
...
The registry is authoritative for table-prefix assignment.

Prefix values are constrained to uppercase alphabetic characters with a length between two and five characters. Uniqueness rules prevent both prefix reuse and duplicate schema/table registrations.
...
All implemented primary keys use clustered physical structures.
...
All currently implemented unique constraints use nonclustered physical structures.
...
The column identifies the constrained column when SQL Server associates the constraint with a single column. A value of `—` indicates that the constraint represents a table-level rule involving multiple columns or expressions.
...
Foreign key definitions are documented using the complete implemented column mapping. Composite relationships preserve the column order defined in SQL Server.

All currently implemented foreign keys are enabled and trusted.
...
Primary key and unique constraint indexes are documented in their respective sections and are therefore not repeated here.

The indexes listed below represent independently defined database indexes.

Filtered unique indexes are used where uniqueness applies only to a subset of rows.
...
Their additional indexes are aligned with the same partition scheme.

For nonunique partitioned indexes, SQL Server may automatically carry the partitioning column in the index structure even when that column was not explicitly declared as an index key.

This occurs in the current implementation for:

| Table | Index | Explicit Key | Partitioning Column |
|---|---|---|---|
...
The implemented partitioning architecture is shared by:

- `sales.Transaction`;
- `sales.TransactionItem`.

Both tables and their partition-aligned indexes use:

- Partition Function: `PF_SALES_MONTHLY`;
- Partition Scheme: `PS_SALES_MONTHLY`;
- Boundary direction: `RANGE RIGHT`;
- Partitioning column in `sales.Transaction`: `TRN_transaction_at`;
- Partitioning column in `sales.TransactionItem`: `TRNIT_transaction_at`.

The partitioning function defines monthly boundaries from January 2025 through January 2028.

Because the partition function uses `RANGE RIGHT`, each boundary value belongs to the partition on its right.
...
The first and last partitions represent open-ended ranges.

`FG_SALES_LEGACY` receives values older than the first defined monthly boundary.

`FG_SALES_FUTURE` receives values at or beyond the final currently defined boundary.
...
The additional destination is:

| Destination ID | Filegroup | Current Role |
|---:|---|---|
| 39 | `FG_SALES_PART_37` | NEXT USED / prepared destination for the next partition split |

`FG_SALES_PART_37` is therefore part of the implemented partitioning architecture even though it is not currently associated with an active partition.

It provides the prepared physical destination required for the next partition boundary introduced through the partition maintenance lifecycle.
...
Current mapped filegroups
    FG_SALES_LEGACY
    FG_SALES_PART_01 ... FG_SALES_PART_36
    FG_SALES_FUTURE

Prepared next destination
    FG_SALES_PART_37
```
...
The triggers documented below are part of the implemented database integrity model.

All currently implemented triggers are enabled and execute as `AFTER` triggers.

| Table | Trigger | Type | INSERT | UPDATE | DELETE | Enabled |
|---|---|---|:---:|:---:|:---:|:---:|
